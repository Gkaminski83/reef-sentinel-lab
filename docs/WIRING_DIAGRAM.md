...
## Complete Wiring Reference: Module 1 + Module 2

Version: 1.0
Date: 2026-03-06
Scope: Phase 1 breadboard prototype

---

## 1. Power Architecture

Each module uses a separate 12V supply and separate LM2596 converter.
- Supply #1 + LM2596 #1 -> Sentinel Hub (Module 1)
- Supply #2 + LM2596 #2 -> Sentinel Chem/Monitor (Module 2)

Critical:
- Set LM2596 output to 5.0V before ESP32 connection.
- Do not power an ESP32 from USB and LM2596 at the same time.
...
- MISO -> GPIO19
- MOSI -> GPIO23
- VCC -> 3V3 (or module-specific supported voltage)
- GND -> GND
...
### Sensors
- pH analog output -> GPIO34
- EC/TDS analog output -> GPIO32
- DS18B20 bus -> GPIO33 with mandatory 4.7k pull-up to 3V3

### Actuators through MOSFETs
- Pump #1 sample -> GPIO15
- Pump #2 HCl -> GPIO2
- Pump #3 waste -> GPIO4
- Pump #4 RO -> GPIO16
- Stirrer PWM -> GPIO25
...
## 4. Common Ground Rule (Critical)

All MOSFET COM/GND pins must share ground with ESP32 ground.
Without a shared ground reference, pump switching signals will fail.

---

## 5. Hydraulic Mapping
- Pump #1: aquarium sample -> measurement chamber
- Pump #2: HCl dosing -> chamber
- Pump #3: chamber -> waste canister
- Pump #4: RO -> chamber/probe storage
- Stirrer below chamber for mixing

Use explicit hose labeling for safety, especially the HCl line.

---

## 6. Pre-Power Checklist
- LM2596 #1 and #2 verified at 5.0V ±0.1V
- No shorts between power rails
- DS18B20 pull-up resistor installed
- Sensor polarity verified
- MOSFET control mapping verified
- Ground continuity confirmed

---

## 7. Common Wiring Issues
- Missing common ground for MOSFET boards
- Reversed TX/RX on UART peripherals
- Wrong resistor value on DS18B20 bus
- Powering 12V pumps from 5V line
- pH BNC routed too close to high-current wiring

---

## 8. Startup Sequence
1. Configure LM2596 modules without ESP32 attached
2. Power down, wire ESP32 and peripherals
3. Flash firmware via USB with external supply disconnected
4. Remove USB, power from 12V path
5. Validate sensors first, then actuators, then chemistry
...
## 6. POŁĄCZENIA HYDRAULICZNE (RURKI/WʯE)
